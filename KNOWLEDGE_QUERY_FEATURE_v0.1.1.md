...
- **双通道检索**：AI 搜索将知识库打包成 `<knowledge_base>` 发送至 `models/gemini-2.5-flash-lite`，并行的直接搜索会返回命中段落，二者统一整理为工具返回内容（纯文本）。
...
- **触发方式**: AI 发起 `query_knowledge` 工具调用并提供 `query` 参数
...
3. **工具调用**: 如果 AI 认为需要查询知识库，会发起 `query_knowledge` 工具调用
4. **参数解析**: 插件检测并解析工具参数
5. **知识库查询**: 异步查询知识库获取相关信息
6. **最终回答**: 以工具返回内容作为 `tool` 消息重新请求 AI，获得更准确的答案
...
AI 现在包含知识库查询指令：
```
如果你需要查询知识库来获取更准确的信息，请调用工具 query_knowledge。
...
工具调用: query_knowledge(query="Minecraft 红石电路基础教程 制作方法")
 ...
 * 处理知识库查询工具调用
 */
private void processKnowledgeQuery(Player player, String query) {
    // 解析工具参数 query
    // 异步查询知识库
    // 重新请求 AI 获得最终答案
}
